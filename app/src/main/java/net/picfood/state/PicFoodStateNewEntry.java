 ...
 */
    ...
    import  net.picfood.ui.PicFoodUI.*;
    ...
    **********************************************************************************************/
    public class PicFoodStateNewEntry extends Activity
    {
        ...
        protected       static          PicFoodStateNewEntry                                singleton                       = null;
        ...
        protected void onCreate( Bundle bundle )
        {
            ...
            (
                ...
                PicFoodActionState.ELeaveNewEntryShowConfirmDialog
            );
            ...
            //reference location text and hide it
            {
                ...
                LibUI.setupTextView( iTextLocation, PicFoodSystems.getFonts().TYPEFACE_BOLD, "" );
            ...
            //reference vicinity text and hide it
            {
                ...
                LibUI.setupTextView( iTextVicinity, PicFoodSystems.getFonts().TYPEFACE_REGULAR, "" );
            ...
            //setup comment label
            {
                ...
                LibUI.setupTextView( this, textCommentExplanation, PicFoodSystems.getFonts().TYPEFACE_REGULAR, R.string.state_new_entry_comment_explanation );
            }

            //reference comment-box and connect to handler
            {
                PicFoodFlowNewEntry.handlerComment.assignField( (EditText)findViewById( R.id.input_field_comment ) );
            }

            //reference button 'submit'
            {
                iButtonSubmit = (Button)findViewById( R.id.submit_entry );
                LibUI.setupButton( this, iButtonSubmit, R.string.state_new_entry_button_submit, PicFoodSystems.getFonts().TYPEFACE_REGULAR, PicFoodActionPush.EPushSubmitNewEntry );
            }

            //reference button 'change location'
            {
                iButtonChangeLocation = (Button)findViewById( R.id.change_location );
                LibUI.setupButton( this, iButtonChangeLocation, R.string.state_new_entry_button_change_location, PicFoodSystems.getFonts().TYPEFACE_REGULAR, PicFoodActionState.EEnterGooglePlacesAndScanGPS );
            }

            //reference button 'recrop image'
            {
                iButtonReCrop = (Button)findViewById( R.id.recrop_image );
                LibUI.setupButton( this, iButtonReCrop, R.string.state_new_entry_button_recrop_image, PicFoodSystems.getFonts().TYPEFACE_REGULAR, PicFoodAction.ELaunchImageCropperNewEntry );
            }

            //reference button 'cancel'
            {
                iButtonCancel = (Button)findViewById( R.id.cancel );
                LibUI.setupButton( this, iButtonCancel, R.string.state_new_entry_button_cancel, PicFoodSystems.getFonts().TYPEFACE_REGULAR, PicFoodActionState.ELeaveNewEntryShowConfirmDialog );
            }
        }
        ...
        public boolean onKeyDown( int keyCode, KeyEvent event )
        {
            switch ( keyCode )
            {
                case KeyEvent.KEYCODE_BACK:
                {
                    ...
                    PicFoodActionState.ELeaveNewEntryShowConfirmDialog.run();
        ...
        private void assignCurrentGatheredData()
        {
            ...
            if ( LibIO.isExistent( PicFoodSD.getFileLastCroppedImageNewEntry() ) )
            {
                try
                {
                    ...
                    Bitmap bmp = LibImage.createBitmapFromFile( this, PicFoodSD.getFileLastCroppedImageNewEntry() );

                    //cut corners and assign
                    if ( Features.CUT_CORNERS_FOR_IMAGES )
                    {
                        //resize to desired displayed width ( fits corner size )
                        bmp = LibImage.resizeBitmapToWidth( bmp, PicFoodImage.getImageSizeToOrder( this, ImageSize.EDetailedImage ) );

                        //cut corners
                        BitmapDrawable bd = PicFoodImage.cutCorners( this, new BitmapDrawable( getResources(), bmp ), ImageSize.EDetailedImage );
                ...
                {
                    PicFoodDebug.DEBUG_THROWABLE( t, "Caught on creating and assigning bitmap from cropped image", UncaughtException.ENo );
                }
            }

            //assign last picked GoogleLocation
            PicFoodDataGooglePlace pickedGooglePlace  = (PicFoodDataGooglePlace)LibIO.stringToSerializable( PicFoodSave.loadSetting( this, SaveKey.EStateNewEntryLastPickedGooglePlace ) );
        ...
        **********************************************************************************************/
        ...
        public void onActivityResult( int requestCode, int responseCode, Intent data )
        {
            ...
            switch ( requestCode )
            {
                case PicFoodSettings.ActivityRequestID.STATE_NEW_ENTRY_PICK_UPLOAD_IMAGE_GALLERY:
                {
                    //unselect buttons
                    ...
                    if ( responseCode == RESULT_OK && data != null )
                    {
                        ...
                        PicFoodFlowNewEntry.handlePickedImage( this, data );
                    }
                    else
                    {
                ...
                case PicFoodSettings.ActivityRequestID.STATE_NEW_ENTRY_PICK_UPLOAD_IMAGE_CAMERA:
                {
                    //unselect buttons
                    //if ( false ) PicFoodActionUnselect.EUnselectButtonsPivotalUpload.run();
                ...
                case PicFoodSettings.ActivityRequestID.STATE_NEW_ENTRY_CROP_IMAGE:
                {
                    //handle selected image data if response code is successful
                    if ( ( responseCode == RESULT_OK && data != null ) )
                    {
                        //handle the new cropped image data
                        PicFoodFlowNewEntry.handleCroppedImage( this );
                    }
                    else
                    {
