    ...
    *   @version    1.0
    **********************************************************************************************/
    public abstract class AntiPatternsImage
    {
        ...
        *****************************************************************************/
        ...
        (
            ...
            final AntiPatternsData accordingAdapterData
        )
        {
            //only perform the order if the image-url is set
            if ( url != null && !LibString.isEmpty( url ) )
            {
                AntiPatternsDebug.imageOrder.out( "Ordering image for base url [" + url + "]" );
                ...
                //check if this ImageView already has a loaded image
                {
                    ...
                    new Thread()
                    {
                        ...
                        public void run()
                        {
                            ...
                            {
                                //replace size in url
                                        int     imageSizeInPixel    = AntiPatternsImage.getImageSizeToOrder(state.getActivity(), imageSize);
                                ...
                                //order this image
                                AntiPatternsDebug.imageOrder.out( " Streaming image from url [" + urlReplaced + "]" );

                                //stream or cache BitmapDrawable
                                BitmapDrawable  bd  = AntiPatternsCache.getBitmapDrawableFromHttpOrCache(state.getActivity(), urlReplaced);

                                //cut corners for images ?!
                                if ( Features.CUT_CORNERS_FOR_IMAGES )
                                {
                                    bd = AntiPatternsImage.cutCorners(state.getActivity(), bd, imageSize);
                                }

                                AntiPatternsDebug.imageOrder.out( " Streamed image is [" + bd + "]" );
                            ...
                            {
                                //ignore Throwable
                                AntiPatternsDebug.imageOrder.out( " Ordering image threw an EXCEPTION [" + t.getClass() + "]" );
                                AntiPatternsDebug.imageOrder.trace( t );
                            }
                        }
                    }.start();
                }
        ...
        *****************************************************************************/
        public static final int getImageSizeToOrder( Context context, AntiPatternsUI.ImageSize imageSize )
        {
            switch ( imageSize )
            {
                case EDetailedImage:
                ...
                case ETiledImage:
                {
                    ...
                    {
                        AntiPatternsDebug.tiledColumnWidth.out( "Initializing tiled image size - All sizes are in px" );
                        ...
                        AntiPatternsDebug.tiledColumnWidth.out( " Available screen width: [" + availableWidth + "]" );
                        ...
                        AntiPatternsDebug.tiledColumnWidth.out( " Minimum tile width: [" + minTiledImageSize + "]" );
                        ...
                        AntiPatternsDebug.tiledColumnWidth.out( " Number of columns: [" + numberOfColumns + "]" );
                        ...
                        AntiPatternsDebug.tiledColumnWidth.out( " Space to distribute: [" + spaceToDistribute + "]" );

                        //pick actual column width
                        int finalColumnWidth = spaceToDistribute / numberOfColumns;
                        AntiPatternsDebug.tiledColumnWidth.out( " Final column width: [" + finalColumnWidth + "]" );
        ...
        **********************************************************************************************/
        public static final void resizeImageToMaxSize( Activity activity, int maxImageSize, File file ) throws Throwable
        {
            ...
            AntiPatternsDebug.resizeImage.out( "  >>> PICKED cropped bitmap, w: [" + lastCroppedBitmap.getWidth() + "] h: [" + lastCroppedBitmap.getHeight() + "]" );
            ...
            if ( lastCroppedBitmap.getWidth() > maxImageSize )
            {
                ...
                AntiPatternsDebug.resizeImage.out( "  >>> RESIZED cropped bitmap, w: [" + lastCroppedBitmap.getWidth() + "] h: [" + lastCroppedBitmap.getHeight() + "]" );

                //save again
                byte[] bytes = LibImage.getBytesFromBitmapAsJPEG( AntiPatternsSettings.Image.DEFAULT_JPEG_QUALITY, lastCroppedBitmap );
        ...
        **********************************************************************************************/
        public static final BitmapDrawable cutCorners( Activity activity, BitmapDrawable bd, AntiPatternsUI.ImageSize imageSize )
        {
            switch ( imageSize )
            {
                case ETiledImage:
