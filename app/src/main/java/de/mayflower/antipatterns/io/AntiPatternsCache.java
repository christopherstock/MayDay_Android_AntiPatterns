    ...
    *   @version    1.0
    **********************************************************************************************/
    public abstract class AntiPatternsCache
    {
        /*********************************************************************************
        ...
        *********************************************************************************/
        public static final BitmapDrawable getBitmapDrawableFromHttpOrCache( Context context, String url ) throws Throwable
        {
            ...
            if ( cacheEnabled )
            {
                ...
                cachedBytes = AntiPatternsSystems.getCache().get( url );
            ...
            if ( bitmap == null )
            {
              ...
              //AntiPatternsDebug.googlePlaces.out( "Received StatusCode [" + statusCode + "]" );
                ...
                AntiPatternsDebug.imageCache.out( "http [" + statusCode + "] streamed [" + bytes.length + "] bytes from url [" + url + "]" );
                ...
                if ( cacheEnabled )
                {
                    try
                    {
                        AntiPatternsSystems.getCache().put( url, bytes );
                    }
                    catch ( Throwable t )
                    {
                    }
            ...
            {
                AntiPatternsDebug.imageCache.out( "cached [" + url + "] created Bitmap from [" + ( cachedBytes == null ? -1 : cachedBytes.length ) + "] bytes" );
            }
        ...
        *   The cache-directory can be obtained using {@link AntiPatternsSD#getCacheDir()}.
        *
        ...
        *********************************************************************************/
        public static final void deleteCache( Activity activity )
        {
            //files
            {
                ...
                LibIO.getAllFileEntries( AntiPatternsSD.getCacheDir(), allFiles );
                ...
                for ( int i = 0; i < allFiles.size(); ++i )
                {
                    ...
                    if ( lastPercentDisplay < percent )
                    {
                        ...
                        AntiPatternsDebug.imageCache.out( "Updated progress bar" );
                    }

                    AntiPatternsDebug.imageCache.out( "Delete cache entry [" + allFiles.elementAt( i ).getAbsolutePath() + "]" );
                    LibIO.deleteFile( allFiles.elementAt( i ) );
                }

                AntiPatternsDebug.imageCache.out( "" );
            ...
            //dirs
            {
                ...
                LibIO.getAllDirectories( AntiPatternsSD.getCacheDir(), allDirs );
                ...
                for ( int i = 0; i < allDirs.size(); ++i )
                {
                    ...
                    AntiPatternsDebug.imageCache.out( "Delete cache dir [" + allDirs.elementAt( i ).getAbsolutePath() + "]" );
                ...
                AntiPatternsActionDialog.EDialogDeleteImageCacheSuccess.run();
            }
        }
    }
