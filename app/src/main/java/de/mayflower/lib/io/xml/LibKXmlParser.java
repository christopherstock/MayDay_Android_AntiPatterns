    ...
    class LibKXmlParser implements XmlPullParser
    {
        ...
        private final boolean adjustNsp() throws XmlPullParserException {
            ...
            for (int i = 0; i < attributeCount << 2; i += 4) {
                ...
                if (cut != -1) {
                    aPrefix = attrName.substring(0, cut);
...
*/
        ...
        private final void parseEndTag() throws IOException,
            ...
            if (!name.equals(elementStack[sp + 3])) {
                error("expected: /" + elementStack[sp + 3] + " read: "
                        + name);
                ...
                int probe = sp;
                while (probe >= 0
                        && !name.toLowerCase( Locale.getDefault() ).equals(
                                elementStack[probe + 3].toLowerCase( Locale.getDefault() ))) {
                    stackMismatch++;
                    probe -= 4;
                }
...
*/
        ...
        private final void pushEntity() throws IOException,
            ...
            if (code.charAt(0) == '#') {
                int c = (code.charAt(1) == 'x' ? Integer.parseInt(code
                        .substring(2), 16) : Integer.parseInt(code
                        .substring(1)));
                push(c);
                return;
            }
            ...
            {
                for ( int i = 0; i < result.length(); i++ ) push(result.charAt(i));
            }
        }
        ...
        @Override
        public int getNamespaceCount(int aDepth) {
            if (aDepth > this .depth)
                throw new IndexOutOfBoundsException();
            return nspCounts[aDepth];
        }
        ...
        public String getNamespace(String aPrefix)
        {
            ...
            for (int i = (getNamespaceCount(depth) << 1) - 2; i >= 0; i -= 2) {
                if (aPrefix == null) {
                    if (nspStack[i] == null)
                        return nspStack[i + 1];
                } else if (aPrefix.equals(nspStack[i]))
                    return nspStack[i + 1];
            }
            return null;
        }
        ...
        @Override
        public String getAttributeValue(String aNamespace, String aName) {

            for (int i = (attributeCount << 2) - 4; i >= 0; i -= 4) {
                if (attributes[i + 2].equals(aName)
                        && (aNamespace == null || attributes[i]
                                .equals(aNamespace)))
        ...
        @Override
        public void require(int aType, String aNamespace, String aName)
                throws XmlPullParserException, IOException {

            if (aType != this .type
                    || (aNamespace != null && !aNamespace
                            .equals(getNamespace()))
                    || (aName != null && !aName.equals(getName())))
                exception("expected: " + TYPES[aType] + " {" + aNamespace
                        + "}" + aName);
        }
        ...
        public void skipSubTree() throws XmlPullParserException,
                IOException {
